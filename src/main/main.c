...
int main() {
  ...
  while (1) {
    ...
    if (!strcmp(format, "circle")) {
      ...
      printf("Object: %s\nX1: %d\nY1: %d\nX2: %d\nY2: %d\nX3: "
             "%d\nY3: %d\n Perimeter %f\n",
             geometry_get_type(&object), cords[0], cords[1], cords[2], cords[3],
             cords[4], cords[5], geometry_get_perimeter(&object));
      ...
      printf("Object: %s\nX: %d\nY: %d\nRadius:  "
             ...
             "%d\n Perimeter %f\n is",
             geometry_get_type(&object), cords[0], cords[1], cords[2],
             geometry_get_perimeter(&object), geometry_get_type(&object2),
             cords2[0], cords2[1], cords2[2], geometry_get_perimeter(&object2));
      if (check)
        printf(" intersecting\n");
      else
